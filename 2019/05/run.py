...
def get_value(
    ...
    *indices: int,
    ...
    Args:
        program:
            The Intcode program to extract value from, given position mode.
        modes:
    ...
    Examples:
        >>> get_value([3, 4, 5, 10, 11, 12], "0", 1)
        11
        >>> get_value([3, 4, 5, 10, 11, 12], "110", 0, 1, 2)
        [10, 4, 5]
        >>> get_value([3, 4, 5, 10, 11, 12], "001", 2, 0, 1)
        [5, 10, 11]

    """
    output = [(program[index] if mode == "1" else program[program[index]])
...
def run_program(program: List[int], inputs: int = 0) -> int:
    """
    ...
    Examples:
        ...
        >>> [run_program(program, inputs) for inputs in [7, 8, 9]]
        [0, 1, 0]
        ...
        >>> [run_program(program, inputs) for inputs in [-1, 0, 1]]
        [1, 0, 1]
        ...
        999
    """
    program = program[:]  # protect the original
    outputs = []
    index = 0
    while True:
        # cast to string to capture leading zeros
        instruction = f"{program[index]:04d}"
        mode, opcode = instruction[:-2], int(instruction[-2:])

        # x[2] = operator(x[0], x[1])
        if opcode in OPERATORS:
            noun, verb = get_value(program, mode, index+1, index+2)
            operator = OPERATORS[opcode]
            program[program[index+3]] = int(operator(noun, verb))
            index += 4

        # x[0] = inputs
        elif opcode == 3:
            program[program[index+1]] = inputs
            index += 2

        # output = x[0]
        elif opcode == 4:
            output = get_value(program, mode, index+1)
            outputs.append(output)
            index += 2

        # index = x[1] if x[0] else index
        elif opcode in (5, 6):
            condition, new_index = get_value(program, mode, index+1, index+2)
...
if __name__ == "__main__":
    ...
    # solution part 1: 9431221
    print("solution part 2:", part2(PROGRAM))
    # solution part 2: 1409363
