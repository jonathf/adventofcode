...
def create_path(line: str) -> Dict[Tuple[int, int], List[int]]:
    r"""
    Construct path in the form of a dictionary.

    Args:
        line:
            Comma separated list with directions and steps. Each element on the
            format ``"[UDLR]\d+"``. Letter represents direction, and the digits
            the number of steps.

    Returns:
        Dictionary where keys are the x-y-coordinates, and the values are an
        list of step counts. A list is used in the case the path crosses
        itself and multiple steps are stored to the same coordinate.

    Examples:
        >>> create_path("R8,U5,L5,D3")  # doctest: +NORMALIZE_WHITESPACE
        {(1, 0): [1], (2, 0): [2], (3, 0): [3], (4, 0): [4], (5, 0): [5],
         (6, 0): [6], (7, 0): [7], (8, 0): [8], (8, 1): [9], (8, 2): [10],
         (8, 3): [11], (8, 4): [12], (8, 5): [13], (7, 5): [14], (6, 5): [15],
         (5, 5): [16], (4, 5): [17], (3, 5): [18], (3, 4): [19], (3, 3): [20],
         (3, 2): [21]}
    """
...
def taxicab_norm(coordinates: Tuple[int, int]) -> int:
    """L-1 distance measure."""
    return abs(coordinates[0])+abs(coordinates[1])


def part1(path1: Dict[Tuple[int, int], List[int]],
          path2: Dict[Tuple[int, int], List[int]]) -> int:
    """Do part 1 of the assignment."""
    common_coord = set(path1) & set(path2)
    common_coord = sorted(common_coord, key=taxicab_norm)
    return taxicab_norm(common_coord[0])


def part2(path1: Dict[Tuple[int, int], List[int]],
          path2: Dict[Tuple[int, int], List[int]]) -> int:
    """Do part 2 of the assignment."""
    def path_distance(coordinate: Tuple[int, int]) -> int:
        return path1[coordinate][0]+path2[coordinate][0]
    common_coord = set(path1) & set(path2)
    common_coord = sorted(common_coord, key=path_distance)
    return path_distance(common_coord[0])


if __name__ == "__main__":
    with open("input") as src:
        PATHS = src.read().split()
    PATHS = create_path(PATHS.pop()), create_path(PATHS.pop())
    print("solution part 1:", part1(*PATHS))
    # solution part 1: 896
    print("solution part 2:", part2(*PATHS))
    # solution part 2: 16524
