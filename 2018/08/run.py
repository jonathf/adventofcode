...
def construct_tree(numbers: Sequence[int]) -> Node:
    """
    Construct tree from sequence of integers.

    Args:
        numbers:
            The input integer to create graph tree from.

    Returns:
        The root node of the graph tree. By recursively iterating each nodes
        `children` attribute, all nodes are accessible.

    Examples:
        >>> numbers = [2, 3, 0, 3, 10, 11, 12, 1, 1, 0, 1, 99, 2, 1, 1, 2]
        >>> construct_tree(numbers)  # doctest: +NORMALIZE_WHITESPACE
        Node(children=(Node(children=(), meta=(10, 11, 12)),
                       Node(children=(Node(children=(), meta=(99,)),),
                       meta=(2,))), meta=(1, 1, 2))
    """
    numbers = (number for number in numbers)
    num_children = next(numbers)
    num_meta = next(numbers)
    children = tuple(construct_tree(numbers) for _ in range(num_children))
    meta = tuple(next(numbers) for _ in range(num_meta))
    return Node(children, meta)


def part1(node: Node):
    """Do part 1 of the assignment."""
    return sum(part1(child) for child in node.children)+sum(node.meta)


def part2(node: Node):
    """Do part 2 of the assignment."""
    if node.children:
        return sum(part2(node.children[idx-1]) for idx in node.meta
...
if __name__ == "__main__":
    with open("input") as src:
        TREE = construct_tree(int(d) for d in src.read().split())
    print("solution part 1:", part1(TREE))
    # solution part 1: 44893
    print("solution part 2:", part2(TREE))
    # solution part 2: 27433
