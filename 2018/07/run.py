...
def part1(dependency_graph: Dict[str, List[str]]) -> str:
    """
    Examples:
        >>> dependency_graph = {"C": ["A", "F"], "D": ["E"], "E": [],
        ...                     "A": ["B", "D"], "F": ["E"], "B": ["E"]}
        >>> part1(dependency_graph)
        'CABDFE'
    """
    out = []
    while dependency_graph:
        ...
        # get freed up tasks
        children = set(sum(dependency_graph.values(), []))
        roots = sorted(node for node in dependency_graph if node not in children)

        # perform a task
        del dependency_graph[roots[0]]
        out.append(roots[0])

    return "".join(out)


def part2(dependency_graph: Dict[str, List[str]], workers: int = 4, penalty: int = 60) -> int:
    """
    Examples:
        >>> dependency_graph = {"C": ["A", "F"], "D": ["E"], "E": [],
        ...                     "A": ["B", "D"], "F": ["E"], "B": ["E"]}
        >>> part2(dependency_graph, workers=2, penalty=0)
        15
    """
    active_workers = {}
    time = 0
    while dependency_graph:
        ...
        # get freed up tasks
        children = set(sum(dependency_graph.values(), []))
        roots = sorted(node for node in dependency_graph
                       if node not in children
                       if node not in active_workers)

        # assign workers
        while roots and len(active_workers) < workers:
            active_workers[roots.pop(0)] = ascii_uppercase.index(roots[0])+penalty+1

        # move time forward
        delta_time = min(active_workers.values())
        time += delta_time

        # update scheduling
        for node in sorted(active_workers):
            active_workers[node] -= delta_time
            if not active_workers[node]:
                del active_workers[node]
...
if __name__ == "__main__":
    with open("input") as src:
        EDGES = re.findall(r"(\w) must be finished before step (\w)", src.read())
    ...
    print("solution part 1:", part1(GRAPH.copy()))
    # solution part 1: BKCJMSDVGHQRXFYZOAULPIEWTN
    print("solution part 2:", part2(GRAPH.copy()))
    # solution part 2: 1040
